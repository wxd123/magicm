...
from magicm.management.config.config_manager import ConfigManager
...
# 全局单例实例
display_command: DisplayCommand = DisplayCommand()
...
def create_display(config_path: Optional[str] = None) -> DisplayCommand:
    """创建显示命令实例（便捷函数）"""
    return DisplayCommand(config_path=config_path)
...
def display_hardware_info(system_info: Dict[str, Any], 
    ...
    """
    if config_path:
        cmd = DisplayCommand(config_path=config_path)
    else:
        cmd = display_command
    cmd.display_all(system_info)
...
def format_hardware_info(system_info: Dict[str, Any], cpu_info: Dict[str, Any],
    ...
    if config_path:
        ...
        cmd = display_command
    return cmd.format_text(system_info, cpu_info, gpu_info, memory_info, width)